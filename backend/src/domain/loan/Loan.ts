...
export class Loan {
  ...
  get id(): string { return this.data.loanId; }
  get status(): LoanStatus { return this.data.status; }
  get amount(): Cents { return this.data.amount; }
  get remainingBalance(): Cents { return this.data.remainingBalance; }
  get borrowerId(): string { return this.data.borrowerId; }
  ...
  approve(score: number, creditLimit: Cents): void {
    ...
    if (this.data.amount > creditLimit) {
      throw new InvariantViolationError(`Loan amount ${this.data.amount} exceeds borrower limit ${creditLimit}`);
    }
